...
void solido::draw(GLenum modo, color c_fondo, bool ajedrez, int borde, color c_borde, float ancho, bool textura,GLuint id)
{
...
//     d=0;
    ...
    for(int j=0;j<ndiv;j++){
      ...
      insertarVertice(x,y,z,tabla_textura(j/(float)(ndiv-1),1-(d[a]/d[nivel-2])));
...
void escenaP4(float a, float b,solido * peon,textura *tex, solido *lata, textura *tex_lata, solido *lata_inf, solido * lata_sup){
  ...
  float pos[]={0,10,2,0};
  fuente_luz *luz=new fuente_luz(color(/*0.2, 0.7, 0.7*//*1, 0.57, 0.2*/1,1,1),0,pos,a,b);
  float pos2[]={3,10,4,0};
//   fuente_luz *luz2=new fuente_luz(color(1,1,1),1,pos2,0,0,GL_LIGHT2);
  fuente_luz *luz2=new fuente_luz(color(1, 0.57, 0.2),0,pos,0,4.39823,GL_LIGHT3);
  float pos3[]={6,10,6,0};
  fuente_luz *luz3=new fuente_luz(color(1,1,1),1,pos3,0,0,GL_LIGHT3);
  float pos4[]={0,0.5,2,0};
  fuente_luz *luz4=new fuente_luz(color(1,1,1),0,pos4,30,30,GL_LIGHT4);
  
  float pos0[]={3.0, 0.0, 3.0, 1.0};
  fuente_luz *luzpos=new fuente_luz(color(/*0.2, 0.7, 0.7*/1,1,1),0,pos0,30,30,GL_LIGHT5);
  ...
  GLfloat mat_ambiente[]={0.05, 0.05, 0.05, 1.0},
  mat_difuso[]={0.7, 0.7, 0.7, 1.0 },
  mat_especular[]={1.0, 1.0, 1.0, 1.0},emision[]={0.3, 0.3, 0.3, 1.0},
  mat_brillo=20;  //*/
  
  GLfloat mat_ambiente_blanco[]={0.05, 0.05, 0.05, 1.0},
  mat_difuso_blanco[]={0.5, 0.5, 0.5, 1.0 },
  mat_especular_blanco[]={0.0, 0.0, 0.0, 0.0},emision_blanco[]={0.6, 0.6, 0.6, 1.0},
  mat_brillo_blanco=6;
 ...
 /* GLfloat mat_ambiente[]={0,0,0,0},
  ...
  material *lata_mat=new material(emision,mat_ambiente,mat_difuso,mat_especular,mat_brillo,tex_lata);
  material *blanco=new material(emision_blanco,mat_ambiente_blanco,mat_difuso_blanco,mat_especular_blanco,mat_brillo_blanco);
  material *madera=new material(NULL,mat_ambiente,mat_difuso,mat_especular,mat_brillo,tex);
  material *perla=new material(NULL,mat_ambiente2,mat_difuso2,mat_especular2,mat_brillo2);
  material *oro=new material(NULL,oro_ambiente,oro_difuso,oro_especular,oro_brillo);
  material *cromo=new material(NULL,cromo_ambiente,cromo_difuso,cromo_especular,cromo_brillo);
  material *negro=new material(NULL,pna,pnd,pne,pnb);
  ...
  glPushMatrix();
//   blanco->activar();
  lata->draw(GL_SMOOTH,color(1,1,1),0,0,color(0,1,1),2,1,tex_lata->getId());
  glPopMatrix();
  lata_mat->desactivar();
